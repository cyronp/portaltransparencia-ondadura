...
import { formatCompetencia } from "@/app/relatorio-prestacao/utils";
...
export default function ReportModal({ isOpen, onClose }: ReportModalProps) {
  ...
  useEffect(() => {
    ...
    const fetchCompetencias = async () => {
      setLoading(true);
      try {
        const res = await fetch("/api/prestacao");
        if (!res.ok) throw new Error("Erro ao buscar períodos");
        const data = await res.json();
        
        if (active) {
          const dates = (data.competencias || []).map((comp: string) => ({
            value: comp,
            label: formatCompetencia(comp),
          }));
          setAvailableDates(dates);

          // Auto-select the first one if not already set
          if (dates.length > 0) {
  ...
  const selectedLabel = availableDates.find(
    ...
    >
      ...
      >
          ...
          <div className="relative">
            <DropdownMenu>
              <DropdownMenuTrigger
                ...
                className="group w-full flex items-center justify-between bg-transparent border-2 border-neutral-700 hover:border-ondadura-yellow-200/60 disabled:opacity-50 disabled:cursor-not-allowed px-4 py-3 cursor-pointer text-sm font-mono tracking-wider uppercase font-bold text-white transition-colors"
              >
        ...
        <button
          ...
          onClick={() => {
            router.push(`/relatorio-prestacao?competencia=${selectedDate}`);
            onClose();
          }}
